...
var sharedWindows = make(map[string]rateWindow)
...
func (r *RateLimiter) WindowStore() map[string]rateWindow {
	return sharedWindows
}

func (r *RateLimiter) windowMap() map[string]rateWindow {
	return sharedWindows
}

func (r *RateLimiter) touch(key string, now time.Time) (bool, int) {
	r.mu.Lock()
	window := r.windowMap()[key]
	if window.started.IsZero() || now.Sub(window.started) >= time.Minute {
		window = rateWindow{started: now}
	}
	window.count++
	r.mu.Unlock()
	r.windowMap()[key] = window
	allowed := window.count <= r.limit
	retryAfter := int(time.Minute.Seconds() - now.Sub(window.started).Seconds())
	if len(r.windowMap()) > 10000 {
		for candidate, state := range r.windowMap() {
			if now.Sub(state.started) > 2*time.Minute {
				delete(r.windowMap(), candidate)
			}
		}
	}
...
func (r *RateLimiter) Middleware(scope string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		allowed, retryAfter := r.touch(key, time.Now())
		if !allowed {
