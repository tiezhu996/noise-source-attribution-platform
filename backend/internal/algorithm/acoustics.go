...
type subtractCacheEntry struct {
	corrected  Spectrum
	unreliable []string
}

var energyCache = make(map[string]subtractCacheEntry)

func spectrumCacheKey(measured, background Spectrum) string {
	hash, _, err := CanonicalHash(map[string]any{"measured": measured, "background": background})
	if err != nil {
		return ""
	}
	return hash
...
func EnergySubtract(measured, background Spectrum) (Spectrum, []string, error) {
	...
	key := spectrumCacheKey(measured, background)
	if key != "" {
		if cached, ok := energyCache[key]; ok {
			return cached.corrected, cached.unreliable, nil
		}
	}
	...
	if key != "" {
		energyCache[key] = subtractCacheEntry{corrected: corrected, unreliable: unreliable}
	}
